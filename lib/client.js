...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** The 💩 burst on 喂食: turds pop out of the mouth area, each slightly
     * smaller, drifting diagonally (--pp-drift-end per turd) and shrinking
     * as it floats up. */
    const POOP_BURST = [
...
@property --pp-drift{syntax:'<length>';inherits:false;initial-value:0px}
.pp-food{position:absolute;left:50%;bottom:30%;--pp-drift-end:0px;line-height:1;pointer-events:none;z-index:4;opacity:0;animation:pp-food 1.25s ease-out forwards}
@keyframes pp-food{0%{opacity:0;--pp-drift:0px;transform:translate(calc(-50% + var(--pp-drift)),6px) scale(.5)}12%{opacity:1;--pp-drift:calc(var(--pp-drift-end) * 0.35);transform:translate(calc(-50% + var(--pp-drift)),-2px) scale(1)}55%{opacity:1;--pp-drift:calc(var(--pp-drift-end) * 0.75);transform:translate(calc(-50% + var(--pp-drift)),-22px) scale(.72)}100%{opacity:0;--pp-drift:var(--pp-drift-end);transform:translate(calc(-50% + var(--pp-drift)),-48px) scale(.35)}}
...
`;
    ...
    function PhotoPetApp(props) {
      ...
      // 喂食: bounce + a 💩 burst from the mouth (2–3 turds drifting
      // diagonally and shrinking as they float up) + a rotating feeding line.
      // The burst clears itself after the longest animation (~1.9s).
      ...
      useEffect(() => {
        if (foodFx === 0) return;
        const timer = window.setTimeout(() => setFoodFx(0), 2000);
