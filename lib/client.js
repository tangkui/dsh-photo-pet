...
`;
     ...
     * offers 更新 (update) / 卸载 (uninstall). Update is disabled while up to
     * date; uninstall is two-step ("卸载" → "确认卸载?") so it cannot fire by
     * accident. After a successful action the GUI restarts automatically. */
      ...
      const status = phase === 'updating' || phase === 'uninstalling'
        ...
        : phase === 'failed'
          ...
          : version !== null && version.installed !== null
            ? (version.upToDate
                ? t('pluginUpToDate')
                : (version.latest !== null ? t('pluginNewer').replace('{latest}', version.latest) : ''))
            : '';
      const onUpdate = () => {
        setPhase('updating');
        pluginManage.update().then((result) => {
          if (result !== null && result.ok === true && result.updated === false) {
            setVersion((previous) => (previous === null ? null : { ...previous, upToDate: true }));
            setPhase('idle');
            return;
          }
      ...
      return React.createElement('div', { className: 'pp-set-field' },
        ...
        React.createElement('div', { className: 'pp-set-actionRow', role: 'group', 'aria-labelledby': id },
          React.createElement('button', {
            ...
            type: 'button',
            className: 'pp-set-action',
            disabled: disabled || busy || (version !== null && version.upToDate === true),
            onClick: onUpdate,
        ...
        React.createElement('p', { className: 'pp-set-hint' },
          (version !== null && version.installed !== null ? t('pluginVersion') + ' v' + version.installed + (status !== '' ? ' · ' + status : '') : (status !== '' ? status : hint)),
        ),
      );
    }
