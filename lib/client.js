...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** The 💩 burst on 喂食: turds pop out near the speech bubble (top of
     * the pet), each slightly smaller, drifting diagonally (--pp-drift-end
     * per turd) and shrinking as it floats up. */
    const POOP_BURST = [
      { dx: -10, drift: -16, delay: 0.00, duration: 1.25, size: 22 },
      { dx: 2, drift: 4, delay: 0.13, duration: 1.45, size: 18 },
      { dx: 12, drift: 18, delay: 0.26, duration: 1.65, size: 15 },
    ];
...
.pp-food{position:absolute;left:50%;top:-6px;line-height:1;pointer-events:none;z-index:4;animation:pp-food 1.25s ease-out forwards}
@keyframes pp-food{0%{opacity:0;transform:translate(-50%,10px) scale(.5)}14%{opacity:1;transform:translate(-50%,-2px) scale(1)}55%{opacity:1;transform:translate(-50%,-18px) scale(.72)}100%{opacity:0;transform:translate(-50%,-46px) scale(.35)}}
...
`;
    ...
    function PhotoPetApp(props) {
      ...
      const [foodFx, setFoodFx] = useState(0);
      ...
      // 喂食: bounce + a 💩 burst floating up from the bubble area (2–3
      // turds drifting diagonally and shrinking as they float up) + a
      // rotating feeding line. The burst clears itself after the longest
      // animation (~1.9s).
      ...
      // Drive the 💩 burst: the CSS animation (pp-food) already gives a
      // guaranteed vertical rise + shrink + fade; WAAPI overrides it with a
      // per-turd DIAGONAL trajectory (translateX drift while rising). Only
      // simple numeric transforms are used so the keyframes parse everywhere;
      // on any failure the CSS fallback still runs.
      useEffect(() => {
        if (foodFx === 0) return;
        const els = Array.from(document.querySelectorAll('.pp-food'));
        for (const el of els) {
          try {
            if (typeof el.animate !== 'function') continue;
            const drift = Number(el.dataset.drift || 0);
            // WAAPI duration/delay are MILLISECONDS; the data attrs store
            // seconds (e.g. "1.25"), so ×1000 or the burst finishes in 1ms.
            const delay = Number(el.dataset.delay || 0) * 1000;
            const duration = Number(el.dataset.dur || 1250) * 1000;
            el.animate([
              { opacity: 0, transform: 'translate(-50%, 10px) translateX(0px) scale(.5)' },
              { opacity: 1, transform: 'translate(-50%, -2px) translateX(0px) scale(1)', offset: 0.14 },
              { opacity: 1, transform: `translate(-50%, -20px) translateX(${drift}px) scale(.72)`, offset: 0.55 },
              { opacity: 0, transform: `translate(-50%, -48px) translateX(${drift}px) scale(.35)` },
            ], { duration, delay, easing: 'ease-out', fill: 'forwards' });
          } catch { /* keep the CSS fallback animation */ }
        }
        const timer = window.setTimeout(() => setFoodFx(0), 2100);
      ...
      return React.createElement('div', {
        ...
        foodFx > 0 && POOP_BURST.map((poop, index) => React.createElement('div', {
          key: 'food-' + foodFx + '-' + index,
          className: 'pp-food',
          'data-drift': poop.drift,
          'data-delay': poop.delay,
          'data-dur': poop.duration,
          style: { left: 'calc(50% + ' + poop.dx + 'px)', fontSize: poop.size + 'px' },
        }, '💩')),
