...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_SETTINGS = Object.freeze({
      ...
      clickLines: '摸摸我～\n主人好呀！\n我在呢～\n今天也要加油哦！\n喵～\n汪！\n嘿嘿，被你发现了\n最喜欢你啦\n要一直陪着我哦\n累了吗？歇会儿吧\n啾～\n呼噜呼噜…',
      feedLines: '啊呜～\n真好吃！\n吃饱啦,谢谢主人～\n吧唧吧唧…\n还要还要！\n嗝～',
      fanMenuItems: 'hide,shrink,rename,photo,cutout,feed,enlarge,reset',
    });
    ...
    /** Split the stored feed-lines string into a list; empty → built-ins. */
    function parseFeedLines(raw) {
      if (typeof raw !== 'string') return FEED_LINES;
      const lines = raw.split('\n').map((line) => line.trim()).filter((line) => line.length > 0);
      return lines.length > 0 ? lines : FEED_LINES;
    }
    ...
    /** Feeding speech pool (fan menu 喂食 button). */
    const FEED_LINES = [
      '啊呜～',
      '真好吃！',
      '吃饱啦,谢谢主人～',
...
.pp-mouth{position:absolute;left:50%;bottom:20%;width:30%;height:0;transform:translateX(-50%);background:radial-gradient(ellipse at center,#3a2417 50%,#180d06 100%);border-radius:50%;z-index:3;animation:pp-mouth 1.05s ease-out forwards}
@keyframes pp-mouth{0%{height:0}22%{height:17%}46%{height:17%}62%{height:0}100%{height:0}}
.pp-fall{position:absolute;left:50%;bottom:20%;font-size:20px;line-height:1;pointer-events:none;z-index:4;animation:pp-fall .55s ease-in forwards}
@keyframes pp-fall{0%{opacity:0;transform:translate(-50%,-150px) rotate(-25deg) scale(.8)}35%{opacity:1}100%{opacity:1;transform:translate(-50%,0) rotate(12deg) scale(.95)}}
.pp-drunk{animation:pp-drunk 1.8s ease-in-out}
@keyframes pp-drunk{0%,100%{transform:none}8%{transform:rotate(-7deg)}16%{transform:rotate(6deg)}24%{transform:rotate(-8deg)}32%{transform:rotate(7deg)}40%{transform:rotate(-6deg) translateY(2px)}50%{transform:rotate(16deg) translateY(5px)}65%{transform:rotate(12deg) translateY(4px)}78%{transform:rotate(18deg) translateY(7px)}90%{transform:rotate(8deg) translateY(3px)}100%{transform:rotate(0) translateY(0)}}
.pp-star{position:absolute;font-size:16px;line-height:1;pointer-events:none;z-index:4;animation:pp-star 1.1s ease-in-out infinite}
@keyframes pp-star{0%{transform:rotate(0) translateY(0) scale(1)}50%{transform:rotate(180deg) translateY(-5px) scale(1.25)}100%{transform:rotate(360deg) translateY(0) scale(1)}}
...
`;
    ...
    function PhotoPetApp(props) {
      ...
      // 喂食 stage machine: null → 'eat' (mouth opens, 💩 drops in) →
      // 'drunk' (swallowed, dizzy wobble) → null. feedSeq bumps per feed so
      // the mouth/poop remount (restarting their CSS animations) even on a
      // rapid re-feed.
      const [feedStage, setFeedStage] = useState(null);
      ...
      // Feeding lines rotate on every 喂食 (fan menu) click.
      const feedLines = useMemo(() => parseFeedLines(settings.feedLines), [settings.feedLines]);
      useEffect(() => {
        if (!working) {
      ...
      // 喂食: the pet's mouth opens, a 💩 drops into it, gets swallowed, and
      // the pet wobbles drunk (dizzy stars) before recovering — plus a
      // rotating feeding line.
      const handleFeed = useCallback(() => {
        setBounce((n) => n + 1);
        setFeedSeq((n) => n + 1);
        const lines = feedLines.length > 0 ? feedLines : FEED_LINES;
        const line = lines[feedLineIdxRef.current % lines.length];
        feedLineIdxRef.current = (feedLineIdxRef.current + 1) % lines.length;
        say(line);
        setFeedStage('eat');
        window.clearTimeout(feedTimerRef.current);
        feedTimerRef.current = window.setTimeout(() => setFeedStage('drunk'), 620);
        feedTimerRef.current = window.setTimeout(() => setFeedStage(null), 2400);
      }, [feedLines, say]);
      useEffect(() => () => window.clearTimeout(feedTimerRef.current), []);
      ...
      const shellClass = 'pp-shell' + (working ? ' pp-working' : '') + (feedStage === 'drunk' ? ' pp-drunk' : '');
      ...
      const menuItems = [
        ...
        { id: 'feed', icon: '🍗', title: '喂食', onClick: () => { setMenuOpen(false); handleFeed(); } },
      ...
      return React.createElement('div', {
        ...
        // Feeding: the mouth opens over the pet's mouth zone, a 💩 drops in,
        // then (after the swallow) the pet wobbles drunk with dizzy stars.
        feedStage !== null && React.createElement('div', { className: 'pp-mouth', key: 'mouth-' + feedSeq }, null),
        feedStage === 'eat' && React.createElement('div', { className: 'pp-fall', key: 'fall-' + feedSeq }, '💩'),
        feedStage === 'drunk' && [
          React.createElement('div', { key: 'star-' + feedSeq + '-1', className: 'pp-star', style: { left: '16%', top: '-14%' } }, '💫'),
          React.createElement('div', { key: 'star-' + feedSeq + '-2', className: 'pp-star', style: { left: '80%', top: '-18%', fontSize: '12px', animationDelay: '.2s' } }, '💫'),
          React.createElement('div', { key: 'star-' + feedSeq + '-3', className: 'pp-star', style: { left: '48%', top: '-28%', fontSize: '20px', animationDelay: '.4s' } }, '💫'),
        ],
    ...
    class PhotoPetCardController {
      ...
      projection() {
        return {
          ...
          feedLines: this.form.field('feedLines'),
    ...
    function PhotoPetSettingsCard(props) {
      ...
      return React.createElement(SettingsCard, {
        ...
        React.createElement(SettingsTextareaField, {
          id: 'photo-pet-feed-lines', label: t('feedLines'), hint: t('feedLinesHint'),
          overriddenLabel: t('overridden'), resetLabel: t('reset'),
          disabled,
          ...state.feedLines,
          onEdit: (text) => props.edit('feedLines', text),
          onReset: () => props.resetField('feedLines'),
        }),
    ...
    const PHOTO_PET_CARD_LOCALE_ZH = {
      ...
      feedLines: '喂食时的话语',
      feedLinesHint: '点菜单"喂食"时气泡轮流显示,每喂一次换下一句,每行一句。',
      fanMenu: '悬浮菜单显示项',
    ...
    const PHOTO_PET_CARD_LOCALE_EN = {
      ...
      feedLines: 'Feeding lines',
      feedLinesHint: 'Rotated in the bubble when you feed (🍗 in the fan menu) — one line per feed, one line each row.',
