...
/**
 * Environment with the common pnpm/dsh locations prepended to PATH. The GUI
 * process can be started from a sanitized shell (e.g. a detached launcher)
 * whose PATH lacks the user's pnpm home — without this the real update /
 * uninstall commands would fail with "pnpm not found".
 */
function pnpmEnv() {
...
/** Run a real command to completion (bounded) and report its outcome. */
function runCommand(file, args, timeoutMs) {
  return new Promise((resolve) => {
    execFileCallback(file, args, { env: pnpmEnv(), timeout: timeoutMs }, (error, stdout, stderr) => {
      if (error) {
        resolve({ ok: false, error: error.code || 'command-failed', detail: String(stderr || stdout || error.message || '').slice(0, 300) })
        return
      }
      resolve({ ok: true })
    })
...
/** Restart the web GUI (default port 3080) after a successful action. */
  ...
  return [
    ...
    }),
    {
      ...
      handler: (req, res) => {
        ...
        void (async () => {
          ...
          // Run the REAL pnpm update first; only restart after it actually
          // succeeded — a failure is reported to the button instead of being
          // masked by a pointless GUI bounce.
          const result = await runCommand('pnpm', ['--dir', profileDirOf(), 'update', 'dsh-photo-pet'], 120000)
          if (!result.ok) {
            writeJson(res, 200, { ok: false, error: 'update-failed', detail: result.detail })
            return
          }
          spawnDetached(restartScript())
    ...
    },
    {
      ...
      handler: (req, res) => {
        if (!requireMethod(req, res, 'POST')) return
        void (async () => {
          const result = await runCommand('dsh', ['plugin', '--profile', profileNameOf(), 'remove', 'dsh-photo-pet'], 120000)
          if (!result.ok) {
            writeJson(res, 200, { ok: false, error: 'uninstall-failed', detail: result.detail })
            return
          }
          spawnDetached(restartScript())
...
export function makePhotoPetSettingsSchema() {
  return z.object({
    ...
    /** Feeding-state bubble lines, one per line; each 喂食 shows the next. */
    feedLines: z.string().default(
      '啊呜～\n真好吃！\n吃饱啦,谢谢主人～\n吧唧吧唧…\n还要还要！\n嗝～',
    ),
    /** Which hover fan-menu items are shown (comma-separated ids). */
    fanMenuItems: z.string().default('hide,shrink,rename,photo,cutout,feed,enlarge,reset'),
  })
}
